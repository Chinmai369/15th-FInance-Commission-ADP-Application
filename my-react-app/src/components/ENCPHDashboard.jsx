...
export default function ENCPHDashboard({
  ...
  useEffect(() => {
    const pending = forwardedSubmissions.filter(
      (s) => {
        ...
        // Check if forwarded to ENCPH by status
        if (statusLower.includes("forwarded to encph")) return true;
        
        // Check if forwarded to ENCPH by section (contains "encph")
        if (sectionLower.includes("encph")) return true;
        
        // Check if status starts with "forwarded to" and section contains "encph"
        if (statusLower.startsWith("forwarded to") && sectionLower.includes("encph")) return true;
  ...
  const confirmBulkReject = () => {
    ...
    setRejectBanner(`${count} work(s) rejected and sent back to SEPH.`);
          ...
          {showBulkApproveModal && (
            <div className="fixed inset-0 bg-black/60 z-50 flex justify-center items-center p-4">
              <div className="bg-white rounded-xl shadow-lg max-w-2xl w-full p-6">
                ...
                <div className="mb-4">
                  <p className="text-sm text-gray-600 mb-2">You are about to approve and forward {selectedItems.length} work(s) to CDMA.</p>
                </div>
                <div>
                  <label className="text-sm text-gray-600 font-medium">Verification Note <span className="text-red-500">*</span></label>
                  <textarea
